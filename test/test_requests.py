...
@responses.activate
def test_graphql_custom_path(mocker: MockerFixture) -> None:
    QUERY = (
        "query ($owner: String!, $name: String!) {\n"
        "    repository (owner: $owner, name: $name) {\n"
        "        description\n"
    ...
    with Client(
        ...
        graphql_url="https://graphql.example.com/api",
    ) as client:
        assert client.graphql(QUERY, {"owner": "jwodder", "name": "ghreq"}) == {
            "data": {"repository": {"description": "You're looking at it!"}}
        }
        assert client.graphql(
            QUERY, {"owner": "jwodder", "name": "nonexistent"}, path="gql"
        ) == {
            "data": {"repository": None},
            "errors": [{"type": "NOT_FOUND", "message": "No such repository"}],
        }
